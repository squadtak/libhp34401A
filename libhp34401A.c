/*
 libhp34401A Ver 1.0 2026-06-22
 (c)2026 squad
*/
...
int is34401A(struct sp_port *port)
{
	...
	char *model_pos = NULL;
	if((model_pos = strchr((char *)buf, ',')) == NULL) return -2;
	if(strtok(model_pos + 1, ",") == NULL) return -3;

	int ret = 0;
	if(strstr(model_pos + 1, "34401A") - (model_pos + 1) == 0 && strlen(model_pos + 1) == strlen("34401A")) ret = 1;
...
{
	int ret = -1;
	if(hp34401ASendCommand(port, command)) goto err0;
...
{
	//printf("[INFO]send command:%s\n", command);

	char sendBuf[strlen(command)];
	memcpy(sendBuf, command, strlen(command));
	sendBuf[strlen(command)] = '\n';

	if(sp_blocking_write(port, sendBuf, strlen(command) + 1, 1000) < 0) return -1;
	usleep(20000);
...
int hp34401APrintText(struct sp_port *port, size_t scrollDelay, char *format, ...)
{
	...
	if(strLen > 12){
		...
		memcpy(commandBuf + strlen("DISP:TEXT \"") + strLen, "\"\r", strlen("\"\r"));
